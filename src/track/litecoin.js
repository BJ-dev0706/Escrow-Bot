...
const LitecoinTransactionTracker = async (address, amount) => {
  try {
    // Get API credentials from config
    const { API_KEY, API_SECRET } = COINBASE_CONFIG;
    
    // Request path for accounts
    const requestPath = '/v2/accounts/litecoin/transactions';
    ...
    console.error("Error checking Coinbase transaction:", error.message);
    throw error;
  }
};
